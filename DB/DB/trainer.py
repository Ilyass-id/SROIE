...
class Trainer:
    ...
    def train_step(self, model, optimizer, batch, epoch, step, **kwards):
        ...
        if step % self.experiment.logger.log_interval == 0:
            ...
            print("--------------------")
            self.l_epoch.append(epoch)
            print("epoch : ",self.l_epoch)
            print("--------------------")
            
            print("--------------------")
            self.l_loss.append(loss.item())
            print("loss : ",self.l_loss)
            print("--------------------")
            
            
            print("--------------------")
            self.l_lr.append(self.current_lr)
            print("learning rate : ",self.l_lr)
            print("--------------------")
            
            print("--------------------")
            print("BCE loss list : ",self.l_bce_loss)
            print("--------------------")
            
            print("--------------------")
            print("Thresh loss list : ",self.l_thresh_loss)
            print("--------------------")
            
            print("--------------------")
            print("L1 loss list : ",self.l_l1_loss)
            print("--------------------")
            
            #fig, ax = plt.subplots(figsize=(10, 7))
            #plt.plot(self.l_epoch, self.l_loss, label = "Training loss")
            #plt.xlabel("epochs")
            #plt.ylabel("Loss")
            ...
            for name, metric in metrics.items():
                ...
                #Data vizualisation : extraction of BCE loss, Thresh Loss, and L1 Loss
                
                if name == "bce_loss":
                    self.l_bce_loss.append(metric.mean().item())
                if name == "thresh_loss":
                    self.l_thresh_loss.append(metric.mean().item())
                if name == "l1_loss":
                    self.l_l1_loss.append(metric.mean().item())
